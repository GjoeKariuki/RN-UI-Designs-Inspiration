  ...
  return (
    ...
    <View style={{width:270, borderRadius:40, backgroundColor:fruit.color(1)}} className="mx-5">
      <View className="flex-row justify-end">
        <TouchableOpacity className="p-3 rounded-full mr-4 mt-4" style={{backgroundColor:"rgba(255,255,255,0.3)"}} onPress={()=>setIsFavorite(!isFavorite)}>
            <HeartIcon size={25} color={isFavorite?fruit.shadow:'white'}/>
        </TouchableOpacity>
      </View>
      <View className="flex-row justify-center"
        style={{shadowColor:fruit.shadow,
                shadowRadius:40,
      ...
      <View className="ml-4 my-4">
        ...
        <Text className="font-bold text-xl text-white shadow tracking-wide">$ {fruit.price}</Text>
      </View>
    </View>
